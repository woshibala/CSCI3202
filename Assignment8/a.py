...
def HMM():
	...
	e.writelines("Emission probability: \n")
	...
	while i < 28:
		...
		while j < 28:
			if i != 1 and j != 1:
				print "P( Xt+1 =",str(unichr(i+95)),"| Xt =",str(unichr(j+95)),") =",t_probability[i][j]
				t.write("P( Xt+1 ="+str(unichr(i+95))+"| Xt ="+str(unichr(j+95))+") ="+str(t_probability[i][j])+"\n")
			j += 1
		i += 1
	t.close()
	...
	while i < 28:
		if i != 1:
			print "initial probability:",str(unichr(i+95)),"=",i_probability[i]
